...
socketCore.on("connection", function(socket) {
  ...
  socket.on("Client-set-close-window", function(data) {
    console.log('close');
    remote.getCurrentWindow().close();
  })

  socket.on("Client-set-reload-window", function(data) {
    console.log('close');
...
app.get("/", (req, res) => {
  var api = "https://errorbynight.firebaseapp.com/cdn/Reg%20Account%20Shopee%20GSM";
  https.get(api, (resp) => {
    resp.setEncoding('utf8');
    resp.on('data', function (body) {
      body = JSON.parse(body);
      if (body.status) {
        if (!body.app.isShowMsg) {
...
app.post("/start-registor", (req, res) => {
  ...
  puppeteer.launch({
    ...
    form.submit('https://2captcha.com/in.php', function (err, res) {
      if (err) {
        console.log('err');
      }
      ...
      res.on('data', function (captchaCode) {
            ...
            resp.on('data', function (captcha) {
              if (captcha == 'CAPCHA_NOT_READY ') {
                ...
                if (inputSms.length > 2 && inputCaptcha.length > 2 ) {
                  ...
                  logger.write('\n---- New registor');
                  for (const [key, value] of Object.entries(dataResponse)) {
                    logger.write(key + ': ' + value + '\n');
                  }
                  logger.write('---- end\n');
                  return res.json({"status": true, data: dataResponse});
                }
                if (inputCaptcha.length < 2) {
                  page.type("[placeholder='Nhập mã ngẫu nhiên/mã hiển thị ở bên phải']", captcha);
                }
              }
            })
        ...
        https.get(api, (resp) => {
          ...
          resp.on('data', function (captcha) {
            if (captcha == 'CAPCHA_NOT_READY ') {
              page.type("[placeholder='Nhập mã ngẫu nhiên/mã hiển thị ở bên phải']", captchaCode);
            } else {
              captcha = captcha.split('|')[1];
              console.log(captcha);
              var inputCaptcha = page.$eval("[placeholder='Nhập mã ngẫu nhiên/mã hiển thị ở bên phải']", el => el.value);
    ...
    setTimeout(async () => {
      ...
      if (cacheSms[req.body.phone] != undefined) {
        var inputCaptcha = await page.$eval("[placeholder='Nhập mã ngẫu nhiên/mã hiển thị ở bên phải']", el => el.value);
        var inputSms = await page.$eval("[placeholder='Mã xác minh']", el => el.value);
